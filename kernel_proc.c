...
/* Initialize a PTCB */
static inline void initialize_PTCB(PTCB* ptcb, Task call, int argl, void* args){
  
  ptcb->argl = argl;
  ptcb->ptcb_thread_task = call;
  ptcb->exited = 0;
  ptcb->detached = 0;
  ptcb->ref_count = 0;
  ptcb->cond_var_joiners = COND_INIT;
  if(args!=NULL){
  ptcb->args = args;
}else {
  ptcb->args = NULL;
...
{ 
  ...
  newproc->cond_var_proc = COND_INIT;
...
rlnode *ptcb_new = rlnode_init(&ptcb->ptcb_rlnode, ptcb);
rlist_push_back(&newproc->ptcb_list, ptcb_new);


  if(call != NULL ) {
    ptcb->ptcb_thread = spawn_thread(newproc, start_main_thread);
    wakeup(ptcb->ptcb_thread);
  }
