...
/* A counter that measures the number of threads are running and going out of the scheduler */
int yield_counter = 0;


//#define  int MAX_POP 15
...
//max size of array of lists including TCBs
#define MAX_SCHED_SIZE 5
//max number of calls yield
...
rlnode SCHED[MAX_SCHED_SIZE];      /* The scheduler array with lists */
...
/* This function increases the priority of all threads(aging) in the scheduler in order to prevent starvation */
/*void increase_priority(TCB* list){
  int length = rlist_len(&list);
...
void prevent_starvation(){
   ...
   increase_priority(&SCHED[4]);
   increase_priority(&SCHED[2]);
   rlist_append(&SCHED[3],&SCHED[4]);
   rlist_append(&SCHED[1],&SCHED[2]);
    int empty = is_rlist_empty(&SCHED[2]);
    if(empty == 1){
      &SCHED[2]=&SCHED[3];
      &SCHED[3]=NULL;
    }
...
{ 
 ...
 /*if(yield_counter==MAX_YIELDS){
  ...
  {
    case SCHED_IO:
      current->priority = 0; // SCHED_IO is high priority cause, that's why we put it on the first list.
      current->cause_of_yield = SCHED_IO; 
    break;
    ...
    case SCHED_QUANTUM:
      if (current->priority == 4) 
        ...
        {
          current->priority++;// if a thread quantum  expired, its priority is dicreased by one.
        }
      current->cause_of_yield = SCHED_QUANTUM;
    break;
    ...
    case SCHED_MUTEX:
      if (current->priority == 4) 
        ...
        {
          current->priority++;// if a thread needs a mutex, its priority is dicreased by one and waits its turn to take its mutex back.
        }
      current->cause_of_yield = SCHED_MUTEX;
    break;

    default:
    current->priority = current->priority; // if a thread quantum did not expires, its priority stays stable
  }
...
void initialize_scheduler()
{
  ...
  for(int i=0; i<MAX_SCHED_SIZE; i++){
